...
import { financeOverview } from "../../../api/finance";
import {
  BarChart,
  Bar,
  XAxis,
...
function FinancialOverviewPage() {
  const { data: snapshot } = useQuery({
    queryKey: ["financeSnapshot"],
    queryFn: financeOverview.getSnapshot,
  });
  ...
  const netCashflow =
    totalSales - (totalExpenses + totalPayroll + totalSupplierPaid);
  ...
  const lastNetCashflow =
    (lastMonth?.revenue ?? 0) - (lastMonth?.expenses ?? 0);
  const growthPct =
    lastNetCashflow !== 0
      ? Math.round(
          ((netCashflow - lastNetCashflow) / Math.abs(lastNetCashflow)) * 100,
        )
      : null;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center panel p-4">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">
            High-level financial snapshot, cash flow analysis, and expenditure
            trends.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="panel p-5 hover:-translate-y-1 hover:shadow-md transition-all">
          <div className="flex justify-between items-center mb-2">
            <div className="text-xs font-bold font-header uppercase tracking-wider text-text-muted">
              Net Cashflow
            </div>
        ...
        <div className="panel p-5 hover:-translate-y-1 hover:shadow-md transition-all">
          ...
          <div className="text-3xl font-bold text-status-success-dark">
        ...
        <div className="panel p-5 hover:-translate-y-1 hover:shadow-md transition-all">
          ...
          <div className="text-3xl font-bold text-status-danger-dark">
            ₦{" "}
            {(
              totalExpenses +
              totalPayroll +
        ...
        <div className="panel p-5 bg-surface-muted/30 hover:-translate-y-1 hover:shadow-md transition-all">
          ...
          <div className="text-3xl font-bold text-status-warning">
            ₦ {totalSupplierOwed.toLocaleString()}
          </div>
          <div className="text-xs text-text-secondary mt-2">
            Supplier Owed Balances
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-surface panel-table lg:col-span-2 flex flex-col h-80 hover:-translate-y-1 hover:shadow-md transition-all">
          <div className="p-4 border-b border-surface-border/50 flex items-center gap-2 text-primary">
            <TrendingUp size={18} />
            <h3 className="font-header font-bold uppercase tracking-wide text-sm">
          ...
          <div className="p-4 flex-1 min-h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  cursor={{ stroke: "rgb(var(--color-surface-border))" }}
                  content={
                    <ChartTooltip
                      formatValue={(val: any) =>
      ...
      <div className="panel p-4 flex flex-col hover:-translate-y-1 hover:shadow-md transition-all">
        <div className="text-sm uppercase tracking-wider font-bold text-text-secondary mb-4 font-header border-b border-surface-border pb-2">
        ...
        <div className="flex-1 overflow-y-auto pr-2 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          <div className="col-span-full h-32 flex flex-col items-center justify-center text-text-muted">
            <Banknote
              size={48}
              className="text-surface-border/50 mb-2"
              strokeWidth={1.5}
            />
            <div className="text-sm font-medium">
              Please visit Supplier Payments to see recent transactions
            </div>
          </div>
        </div>
      </div>
