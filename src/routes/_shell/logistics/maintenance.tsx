...
const columnHelper = createColumnHelper<MaintenanceLogEntry>()
...
const columns = [
  ...
  columnHelper.accessor('odometerReading', { header: 'Odometer' }),
...
function MaintenancePage() {
  ...
  const filteredData = data?.filter(row => {
    const matchesSearch = row.truckNo.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesType = typeFilter === 'All' || row.type === typeFilter
    return matchesSearch && matchesType
  }) || []

  const totalCost = filteredData.reduce((acc, row) => acc + row.cost, 0)
  const fuelCost = filteredData.filter(r => r.type === 'fuel').reduce((acc, row) => acc + row.cost, 0)
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold font-header tracking-tight text-primary">Fleet Maintenance & Fuel</h2>
          <p className="text-sm text-text-secondary mt-1">Track operational expenses and repair logs for all trucks.</p>
        </div>
        <button
      ...
      <div className="bg-white p-3 rounded-md shadow-sm border border-surface-border flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex gap-6">
          <div>
            <div className="text-[0.65rem] uppercase tracking-wider font-bold text-text-muted font-header">Total Expense</div>
            <div className="text-lg font-bold text-primary">₦ {totalCost.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-[0.65rem] uppercase tracking-wider font-bold text-text-muted font-header">Fuel Costs</div>
            <div className="text-lg font-bold text-status-warning">₦ {fuelCost.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-[0.65rem] uppercase tracking-wider font-bold text-text-muted font-header">Repairs</div>
            <div className="text-lg font-bold text-status-error">₦ {maintenanceCost.toLocaleString()}</div>
          </div>
        </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Maintenance Log">
        ...
        <form
          ...
          className="space-y-4"
        >
          <form.Field name="truckNo" children={(field) => (
            <SelectField field={field as any} label="Truck No" options={truckOptions} />
          )} />
          
          <form.Field name="type" children={(field) => (
            <SelectField field={field as any} label="Type" options={[
          ...
          <form.Field name="cost" children={(field) => <FormField field={field as any} label="Cost" type="number" />} />
          <form.Field name="odometerReading" children={(field) => <FormField field={field as any} label="Odometer Reading" type="number" />} />
          <form.Field name="notes" children={(field) => <FormField field={field as any} label="Notes (Optional)" />} />
